# Legacy Job Wrapper with Step Functions

This application is a template to wrap a long running legacy batch application behind a 
modern API. It reduces the load on the legacy system as well as providing a persistence
layer for the job status in case the Legacy Job only provides ephemeral storage. In addition Step-Functions built-in Error Handling mechanisms (e.g.retry with back-off) can help you in encapsulating the interaction with the legacy system.
...
```bash
aws cloudformation delete-stack --stack-name async-job
```
